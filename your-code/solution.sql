...
/*Challenge 3*/
SELECT  publication.authors.au_id as Author_ID, publication.authors.au_lname as FName, publication.authors.au_fname as LName, newtable2.Total 
FROM	publication.authors
INNER JOIN 
...
/*Challenge 4*/
SELECT  publication.authors.au_id as Author_ID, publication.authors.au_lname as FName, publication.authors.au_fname as LName, coalesce(newtable2.Total,0)
FROM	publication.authors
LEFT JOIN
(SELECT publication.titleauthor.au_id AS Author_ID, newtable.Title_ID, newtable.Title, newtable.Total
FROM publication.titleauthor
...
(SELECT publication.titles.title_id as Title_ID, publication.titles.title as Title, publication.titles.ytd_sales as Total
FROM publication.titles) AS newtable
ON publication.titleauthor.title_id = newtable.Title_ID
GROUP BY Author_ID) as newtable2
ON publication.authors.au_id=newtable2.Author_ID
ORDER BY coalesce(newtable2.Total,0) DESC;
